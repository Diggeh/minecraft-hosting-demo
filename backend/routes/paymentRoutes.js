...
router.get("/:id/status", async (req, res) => {
    try {
        ...
        if (!payment) {
            console.error(`❌ Status check failed: Payment ${req.params.id} not found.`);
...
// Scan endpoint
router.get("/scan/demo", async (req, res) => {
...
router.get("/scan/:id", async (req, res) => {
    ...
    try {
        ...
        logToFile(`✅ Payment ${payment._id} confirmed! Kicking off server creation...`);
        ...
        logToFile(`🏗️ Details: Name="${serverName}", Port=${randomPort}, User=${payment.userId}`);
        ...
        try {
            ...
            if (craftyError.response) {
                ...
                logToFile(`Response Data: ${JSON.stringify(craftyError.response.data)}`);
            } else {
